...
class CustomSelect extends Component {
  ...
  handleSearchText = (event) => {
    ...
    const filterData = optionsData.filter((item) => {
      ...
      return str.indexOf(event.target.value) !== -1;
    });
  ...
  handleOptionSelection = (event) => {
    ...
    if (optionSelected) {
      const filterSelecteOption = selectedOption.filter((item) => {
  ...
  render() {
    ...
    return (
      ...
      >
        <div
          data-test="component-CustomSelect"
          className="tag-ctn tag-ctn-bootstrap-focus"
        >
          <div className="tag-sel-ctn">
            <Tag
              ...
              handleTagRemove={(option) => {
                this.handleOptionDeSelect(option);
              }}
            />
            <InputTextBox
