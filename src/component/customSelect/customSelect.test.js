...
describe("if props are correctly passed", () => {
  ...
  test("renders without error", () => {
    const customSelectComponent = findByTestAttr(
      wrapper,
  ...
  test("input text box is visible initial", () => {
    const customeSelectInput = findByTestAttr(
      wrapper,
  ...
  test("options list should not be visisble", () => {
    const customeSelectOptionList = findByTestAttr(
      wrapper,
...
describe("select component option list events", () => {
  test("select option, should update selectedOption state", () => {
    ...
    const customSelectOptionList = findByTestAttr(
      wrapper,
      "custom-select-options-list"
    );
    const option = [{ label: "abc", value: "2" }];
    customSelectOptionList.prop("handleOptionSelection")({
      target: { value: "2" },
    });
